 ...
 *
 * @param[in] encChn 编码Channel号,取值范围: [0, @ref NR_MAX_ENC_CHN - 1]
 * @param[in] iTargetBitRate 目标码率，iMaxBitRate 最大码率。注意码率单位是:"bit/s"。
 *
 * @retval 0 成功
 * @retval 非0 失败
 *
 ...
 * 设置码流buffer的大小.
 *
 * @param[in] encChn       通道编号.
 * @param[in] nrStreamSize  码流buffer的大小.
 *
 * @retval  >=0 && < 32    成功.
 * @retval  <0			   失败.
 *
 * @remarks   此API必须在通道创建之前调用.
 *
 * @attention 无.
 */
 ...
 *
 * @param[in] encChn       通道编号.
 * @param[out] nrStreamSize  码流buffer的大小.
 *
 * @retval  >=0 && < 32    成功.
 * @retval  <0			   失败.
 *
